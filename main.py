...
class Contacts(db.Model):
    ...
    name = db.Column(db.String(80), nullable=False) #nullable means field can't be empty
...
class Posts(db.Model):
    sno = db.Column(db.Integer, primary_key=True)
    slug = db.Column(db.String(25), nullable=False) #nullable means field can't be empty
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_username']):
        if request.method == "POST":
            ...
            if sno == "0":
                post = Posts(title = box_title, slug = slug, content= content, img_file = img_file, date=datetime.now(), tagline = tagline)
                db.session.add(post)
                db.session.commit()
